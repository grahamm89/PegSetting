...
// --- One-time SW self-heal: unregister old SWs and clear old caches if loading fails previously ---
(function(){
  const KEY = 'swSelfHealDone';
  ...
  try{
    if (!localStorage.getItem(KEY)){
      navigator.serviceWorker.getRegistrations().then(regs => {
        ...
        console.log('[SW] self-heal executed once');
      });
    }
  }catch(e){}
})();

// Admin button + #admin trigger
(function(){
  const btn = document.getElementById('openAdminBtn');
  if (btn && typeof openOverlay === 'function'){
    btn.addEventListener('click', openOverlay);
  }
  if (location.hash === '#admin' && typeof openOverlay === 'function'){
...
(function(){
  ...
  const dbg = (location.hash === '#debug') || q('debug') === '1';
  const panel = document.getElementById('diagPanel');
  const logEl = document.getElementById('diagLog');
  const closeBtn = document.getElementById('diagClose');
  const bypassBtn = document.getElementById('bypassSW');

  function show(){ if (panel) panel.style.display = 'block'; }
  function hide(){ if (panel) panel.style.display = 'none'; }
  function log(msg){ console.log('[diag]', msg); if (logEl){ logEl.textContent += (msg + '\n'); logEl.scrollTop = logEl.scrollHeight; } }

  if (closeBtn) closeBtn.onclick = hide;
  if (bypassBtn) bypassBtn.onclick = () => {
    try { localStorage.setItem('swBypassUntil', String(Date.now() + 10*60*1000)); } catch(e){}
    alert('Service Worker bypass enabled for 10 minutes. Reloading...');
    location.reload();
  };

  if (dbg){ show(); }
  window.__diag = { show, log };

  // Boot checks
  try {
    if (dbg) show();
    log('Boot start');
    log('Has #adminOverlay: ' + !!document.getElementById('adminOverlay'));
    log('Has dataTable: ' + !!document.querySelector('#dataTable tbody'));
    log('openOverlay typeof: ' + (typeof openOverlay));
    log('state.data present: ' + !!(window.state && Array.isArray(window.state.data)));
  } catch(e){
